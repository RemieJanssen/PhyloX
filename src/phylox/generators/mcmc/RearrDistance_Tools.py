...
def RandomMove(network,available_tree_nodes=[], available_reticulations=[],type_probs = {"horizontal":0.8,"vertPlus":0.1,"vertMin":0.1}):
    edges = list(network.edges())
    movetype = random.choices(tuple(type_probs.keys()), weights=type_probs.values(), k=1)[0]
    if movetype=="horizontal":
        edge_indices = np.random.choice(range(len(edges)),2,replace=True)
        moving_edge = edges[edge_indices[0]]
        to_edge = edges[edge_indices[1]]
        endpoint = random.choice(moving_edge)
        return "horizontal", moving_edge,endpoint,to_edge
    elif movetype=="vertPlus":
        if len(available_tree_nodes)>0:
            edge_indices = np.random.choice(range(len(edges)),2,replace=True)
            edge1 = edges[edge_indices[0]]
            edge2 = edges[edge_indices[1]]
            tree_node_index = np.random.choice(range(len(available_tree_nodes)),1,replace=False)
            tree_node = available_tree_nodes[tree_node_index[0]]
            retic_node_index = np.random.choice(range(len(available_reticulations)),1,replace=False)
            retic_node = available_reticulations[retic_node_index[0]]
            return "vertPlus", edge1,edge2,tree_node,retic_node
        else:
            return "noMove"
    elif movetype=="vertMin":
        edge_indices = np.random.choice(range(len(edges)),1,replace=False)
        removed_edge = edges[edge_indices[0]]
        return "vertMin", removed_edge
...
def CheckValidHorizontal(network, moving_edge, moving_endpoint, to_edge):
    """
...
def DoMove(network, move, check_validity=True):
    """
    ...
    movetype = move[0]
    if movetype=="horizontal":
        _, moving_edge, moving_endpoint, to_edge = move
        # Check wether the move is valid if the user wants to.
        if check_validity:
            if not CheckValidHorizontal(network, moving_edge, moving_endpoint, to_edge):
                return False
        ...
        _, edge1,edge2,tree_node,retic_node = move
        if nx.has_path(network, edge2[1], edge1[0]) or edge1==edge2:
            return False
        else:
            new_network = network.copy()
            new_network.remove_edges_from([edge1,edge2])        
            new_network.add_edges_from([(edge1[0], tree_node), (tree_node, edge1[1]),(edge2[0], retic_node), (retic_node, edge2[1]), (tree_node,retic_node)])
            return new_network
    elif movetype=="vertMin":
        _, removed_edge = move
        if CheckMovable(network, removed_edge, removed_edge[0]) and CheckMovable(network, removed_edge, removed_edge[1]):
            parent_0 = Parent(network, removed_edge[0], exclude=[removed_edge[1]])
            child_0 = Child(network, removed_edge[0], exclude=[removed_edge[1]])
            parent_1 = Parent(network, removed_edge[1], exclude=[removed_edge[0]])
            child_1 = Child(network, removed_edge[1], exclude=[removed_edge[0]])
            if parent_0==parent_1 and child_0==child_1:
                return False
            new_network = network.copy()
            new_network.remove_edges_from([(parent_0, removed_edge[0]), (removed_edge[0], child_0),(parent_1, removed_edge[1]), (removed_edge[1], child_1), removed_edge])
    ...
    for moving_edge in network.edges():
        for to_edge in network.edges():
            for end in headOrTail:
                if CheckValidHorizontal(network, moving_edge, moving_edge[end], to_edge) and moving_edge[
...
def GL_Case1_rSPR(N, Np, up, isom_N_Np, isom_Np_N, randomNodes=False):
    ...
    for parent in N.predecessors(v):
        if CheckValidHorizontal(N, (parent, v), v, (z, x)):
            if not randomNodes:
                u = parent
                break
...
def GL_Case3(N, Np, up, isom_N_Np, isom_Np_N, randomNodes=False):
    ...
    if CheckValidHorizontal(N, (z_x, x), z_x, (z_y, y)):
        return [((z_x, x), z_x, (z_y, y))], [], z_x, up
    # Case3bii: (z_y,y) is movable
    if CheckValidHorizontal(N, (z_y, y), z_y, (z_x, x)):
