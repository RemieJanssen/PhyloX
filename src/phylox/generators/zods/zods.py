...
def generate_network_zods(time_limit, speciation_rate, hybridization_rate):
    ...
    while current_time<time_limit:
        if random.random() < current_speciation_rate / rate:
            ...
            multree.add_weighted_edges_from([(splitting_leaf,current_node,0),(splitting_leaf,current_node+1,0)], weight = 'length')
            ...
            if pl0 != pl1:
                no_of_hybrids+=1
                multree.add_weighted_edges_from([(l0,current_node,0)], weight = 'length')
                leaves.remove(l0)
                leaves.remove(l1)
                leaves.add(current_node)
