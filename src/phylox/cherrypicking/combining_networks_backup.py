...
def Newick_To_Tree(newick, current_labels=dict()):
    ...
    edges, leaves, current_labels, current_node = NestedList_To_Tree(
        nestedtree, 2, current_labels, distances=distances
    )
        ...
        for i in range(0, len(nestedList), 2):
            ...
            if type(t) == list:  # Not a leaf
                ...
                (
                    ...
                    t, current_node, current_labels, distances=distances
                )
            else:  # A leaf
        ...
        for t in nestedList:
            if type(t) == list:
                ...
                (
                    ...
                    current_labels,
                    current_node,
                ) = NestedList_To_Tree(t, current_node, current_labels)
            else:
                if str(t) not in current_labels:
                    current_labels[str(t)] = -len(current_labels)
...
class Input_Set:
    def __init__(self, newick_strings=[]):
        ...
        for n in newick_strings:
            ...
            self.labels, distances_in_tree = tree.Tree_From_Newick(
                newick=n, current_labels=self.labels
            )
    ...
    def Improve_Sequence(self, CPS, reduced_trees, progress=False):
        ...
        while i < len(seq):
            ...
            for j in relevant_tree_indices:
                ...
                new_relevant_pairs_for_trees[j] = Sequence_Reduces_Tree(
...
class PhT:
    ...
    def Tree_From_Newick(self, newick=None, current_labels=dict()):
        self.nw, self.leaves, current_labels, distances = Newick_To_Tree(
            newick, current_labels
        )
    ...
    def Clean_Node(self, v):
        if self.nw.out_degree(v) == 1 and self.nw.in_degree(v) == 1:
            ...
            if "length" in self.nw[pv][v] and "length" in self.nw[v][cv]:
                self.nw[pv][cv]["length"] = (
                    self.nw[pv][v]["length"] + self.nw[v][cv]["length"]
                )
...
class CutTree:
                ...
                if self.nw.in_degree(node) > 1:
                    ...
                    for i, parent in enumerated_parents:
                        if i == 0:
                            ...
                            self.nw.add_edges_from(
                                [(parent, self.current_node, self.nw[parent][node])]
                            )
    ...
    def Newick_Recursive(self, root, probabilities=False):
        ...
        for v in self.nw.successors(root):
            Newick += (
                self.Newick_Recursive(v, probabilities)
                + ":"
                + str(self.nw[root][v]["length"])
            )
...
class CherryPickingMixin:
            ...
            if reduced_trees:
                for i, pair in enumerate(reversed(seq)):
                    if heights:
                        self.add_pair(
                            *pair,
                            red_trees=reduced_trees[total_len - 1 - i],
                            current_trees=current_trees_embedded,
                            height=heights[total_len - 1 - i]
                        )
                        current_trees_embedded = (
                            current_trees_embedded | reduced_trees[total_len - 1 - i]
                        )
                    else:
                        self.add_pair(
                            *pair,
        ...
        for node in self.nw.nodes:
            ...
            if self.nw.in_degree(node) > 1:
                ...
                for parent in self.nw.predecessors(node):
                    ...
                    self.nw[parent][node]["probability_all"] = self.nw[parent][node][
                        "prob"
                    ]
    ...
    def add_pair(self, x, y, red_trees=set(), current_trees=set(), height=[1, 1]):
        ...
        self.nw.add_edge(
            node_y,
            self.no_nodes,
            no_of_trees=no_of_trees_incoming_y + len(red_trees - current_trees),
            length=height_pair_y_real,
        )
        self.nw[parent_node_y][node_y]["length"] = (
            length_incoming_y - height_pair_y_real
        )
        ...
        # x is already in the network, so create a reticulate cherry (x,y)
            ...
            # create a new reticulation vertex above x to attach the hybrid arc to
                ...
                self.nw.add_edge(
                    ...
                    node_x,
                    no_of_trees=len(red_trees),
                    length=height_goal_x - height_pair_x,
                )
                self.nw.add_edge(
                    node_x,
                    self.no_nodes + 1,
    ...
    def SelectSubNetworkByReticulations(
        self, type_is_probability=True, reticulations=0
    ):
        ...
        for i in range(reticulations):
            ...
            for e in self.nw.edges:
                if (not restrictedNetwork.nw.has_edge(e[0], e[1])) and self.nw[e[0]][
                    e[1]
    ...
    def Is_Ret_Cherry(self, x_label, y_label):
        ...
        for p in self.nw.predecessors(y):
            py = p
        return (
            (self.nw.in_degree(px) > 1)
            and self.nw.out_degree(px) == 1
            and (py in self.nw.predecessors(px))
        )
    ...
    def reduce_pair(self, x_label, y_label):
        ...
        if self.Is_Cherry(x_label, y_label):
            self.reducible_pairs.difference_update(
                set([(x_label, y_label), (y_label, x_label)])
            )
            self.nw.remove_node(x)
            ...
            new_pairs = (
                set([("no_leaf", "no_leaf")])
                | self.Find_Pairs_With_First(y_label)
                | self.Find_Pairs_With_Second(y_label)
            )
            self.reducible_pairs = self.reducible_pairs.union(
                new_pairs - set([("no_leaf", "no_leaf")])
            )
            return new_pairs
        if self.Is_Ret_Cherry(x_label, y_label):
            ...
            new_pairs = (
                set([("no_leaf", "no_leaf")])
                | self.Find_Pairs_With_First(x_label)
                | self.Find_Pairs_With_Second(x_label)
                | self.Find_Pairs_With_First(y_label)
                | self.Find_Pairs_With_Second(y_label)
            )
            self.reducible_pairs = self.reducible_pairs.union(
    ...
    def Find_Pairs_With_Second_Separated(self, x_label):
        ...
        return cherries - set([(x_label, x_label)]), retic_cherries - set(
            [(x_label, x_label)]
        )
    ...
    def Find_All_Reducible_Pairs(self):
        ...
        for l in self.leaves:
            self.reducible_pairs = self.reducible_pairs.union(
                self.Find_Pairs_With_First(l)
            )
            self.reducible_pairs = self.reducible_pairs.union(
                self.Find_Pairs_With_Second(l)
            )
    ...
    def Find_All_Reducible_Pairs_Separated(self):
        ...
        for l in self.leaves:
            (
                added_cherries,
                added_retic_cherries,
            ) = self.Find_Pairs_With_Second_Separated(l)
            self.cherries = self.reticulated_cherries.union(added_cherries)
            self.reticulated_cherries = self.reticulated_cherries.union(
                added_retic_cherries
            )
